...
func TestUnmarshal(t *testing.T) {
	for i, item := range tUnmarshalTests {
		vt := reflect.ValueOf(item.value).Type()
		var value interface{}
		switch vt.Kind() {
		case reflect.Map:
			value = reflect.MakeMap(vt).Interface()
		case reflect.String:
			vt := reflect.ValueOf(item.value).Type()
			v := reflect.New(vt)
			value = v.Interface()
...
func TestUnmarshalWithPointerSetter(t *testing.T) {
	for i, item := range setterTests {
...
func TestUnmarshalWithValueSetter(t *testing.T) {
	for i, item := range setterTests {
		obj := &setterValueType{}
		err := Unmarshal([]byte(item.data), obj)
		if err != nil {
...
func TestUnmarshalWithFalseSetterIgnoresValue(t *testing.T) {
...
//}
//
//func BenchmarkUnmarshal(t *testing.T) {
//	var err error
//	for i := 0; i < c.N; i++ {
...
//}
//
//func BenchmarkMarshal(t *testing.T) {
//	var v map[string]interface{}
